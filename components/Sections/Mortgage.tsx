...
const Mortgage = ({}: MortgageProps) => {
  return (
    <section className={`max-w-zhk_max mx-auto pb-8`}>
      <SectionTitle title={MORTGAGE.title} />
      <div className={`h-[230px]`}>
        <QueryContainers.M1 />
      </div>
      <div>
        <h2 className={styles.mortgageBottomRow + ` pb-4`}>{MORTGAGE.bottomRow.title}</h2>
        <div className={`flex w-full`}>
          {MORTGAGE.bottomRow.items.map((item, i) => (
            <div key={`item${i}`} className={`grow flex items-center`}>
              <div className={styles.mortgageBottomRowNumber + ` h-[60px] aspect-square bg-[#52D8AF] flex flex-center justify-center items-center shadow-md`}>
                {i+1}
              </div>
              <Text className={styles.mortgageBottomRowItem + ` px-4 whitespace-nowrap`}>{item}</Text>
              {i !== MORTGAGE.bottomRow.items.length - 1 && <LocalSvg.LongArrowRight />}
            </div>
          ))}
        </div>
      </div>
