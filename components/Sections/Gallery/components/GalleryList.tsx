...
const GalleryList = ({ data, onClick }: GalleryListProps) => {
  return (
    <div>
      {data?.map((section, i) => {
        return (
          <div key={`section${i}`} className={`max-w-zhk_max w-full mx-auto flex flex-col pt-[143px]`}>
            <div className={`flex`}>
              <div className={`w-2/12 flex items-center justify-center pr-6`}>
                <div
                  className={`cursor-pointer rounded-full border-2 border-[#52D8AF] w-full h-auto aspect-square flex items-center justify-center`}
                  onClick={() => onClick(i)}
                >
                  <svg
                    width='123'
              ...
              <div className={`w-8/12 aspect-[1/0.7] h-auto relative cursor-pointer`} onClick={() => onClick(i)}>
                <Image
                  alt={'ЖК Космос'}
